...
object nodesVar7m extends SessionVar[IndexedSeq[Bbox]](IndexedSeq.empty[Bbox])

class ActiveOcrStep7m extends StatefulSnippet {
  ...
  val firstString = "/activeocr7m?count=0" // WHY '/'?
  val prevCount = if (count > 0) count - 1 else 0
  val prevString = "activeocr7m?count=" + prevCount.toString // WHY NO '/'?
  // not enough information to initialize last, next
  var lastString = ""
  var nextCount = 0
  var nextString = ""
  var ocrText = ""
  if (nodesVar7m.is.isEmpty) {
    nodesVar7m(pages.head.bbList)
  }
  val nodes = nodesVar7m.is
  for (page <- pages) {
    // enough information to initialize last, next
    val lastCount = nodes.length - 1
    lastString = "activeocr7m?count=" + lastCount.toString
    nextCount = if (count < lastCount) count + 1 else lastCount
    nextString = "activeocr7m?count=" + nextCount.toString
  ...
  def updateAt(i: Int, correction: String) = {
    ...
    nodesVar7m(nodes.updated(i, updatedNode))
  }
